...
logger = logging.getLogger("SuperPages")
...
async def extract_business_details(container: Locator) -> Dict[str, str] | None:
    ...
    try:
        ...
        if await address.count() > 0:
            address = (await address.first.text_content()).strip()
            split_address = address.rsplit(",", maxsplit=2)
            clean_address = split_address[0]
            if len(split_address) == 2:
                clean_address += "," + re.sub(r'[^a-zA-Z]+', '', split_address[-1]) # state name
            elif len(split_address) == 3:
                clean_address += "," + split_address[1].title().strip()
                clean_address += "," + split_address[2].upper().strip()
