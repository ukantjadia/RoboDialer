...
logger = logging.getLogger("YellowPages USA")
...
async def extract_businesses(page):
    ...
    for listing in listings:
        ...
        try:
            address_parts = []
            street = await listing.query_selector('div.street-address')
            if street:
                street_text = (await street.inner_text()).strip()
                address_parts.append(street_text)
            
            locality = await listing.query_selector('div.locality')
            if locality:
                city_state = (await locality.inner_text()).strip()
                city_state = re.sub(r'[0-9]+', '', city_state)  #remove postal code
                address_parts.append(city_state)
            
            if address_parts:
                business_info['Address'] = ", ".join(address_parts)
            else:
                full_address = await listing.query_selector('.adr')
                if full_address:
