...
from backend.services.preprocessing.parser import parse_number
...
logger = logging.getLogger("Hotfrog USA")
...
async def extract_business_details(container: Locator, search_term: str) -> Dict[str, str] | None:
    try:
        ...
        address_loc = container.locator("div.col-8 > span.small")
        address = (await address_loc.inner_text()).strip() if await address_loc.is_visible() else "NA"
        address = " ".join(address.split()).replace("\xa0", " ")
        address = ",".join(address.rsplit(",",maxsplit=3)[:-1])
...
async def extract_businesses(page, search_term: str) -> List[Dict[str, str]]:
    ...
    if await main.count() > 0:
        containers = main.locator('div.hf-box')
...
async def main_test():
    ...
    location_to_test = "houston, tx"
    pages_to_scrape = 2
    
    start_time = time.time()
    scraped_results = []
    try:
        async for record in scrape_hotfrog(
            search_term=industry_to_test, 
