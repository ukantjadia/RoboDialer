...
export const CallProvider = ({ children }: { children: ReactNode }) => {
 ...
 const createOrUpdateCallOnBackend = useCallback(async (call: Call | null) => {
  ...
  const startOutgoingCall = useCallback(async (to: string, contactName?: string, leadId?: string) => {
    ...
    console.log('🔍 StartOutgoingCall - Current agent:', agent);
    ...
    if (!agent.id) {
        console.error('🔍 StartOutgoingCall - Agent has no ID:', agent);
        toast({ title: 'Agent Error', description: 'Agent ID is missing. Please re-login.', variant: 'destructive' });
        return;
    }
    
    console.log('🔍 StartOutgoingCall - Agent ID:', agent.id);
    
    dispatch({ type: 'SET_SOFTPHONE_OPEN', payload: true });

    const formattedNumber = formatUSPhoneNumber(to);
    console.log('🔍 StartOutgoingCall - Formatted number:', formattedNumber);
    ...
    const requestBody = { 
      agent_id: Number(agent.id), // Ensure it's a number
      to: formattedNumber, 
      dialer_type: 'manual' 
    };
    console.log('🔍 StartOutgoingCall - Request body:', requestBody);
    ...
    try {
        ...
        console.log('🔍 StartOutgoingCall - Response status:', response.status);
        console.log('🔍 StartOutgoingCall - Response ok:', response.ok);
        
        if (!response.ok) {
            const error = await response.json();
            console.error('🔍 StartOutgoingCall - Error response:', error);
  ...
  const getAgentPreferences = useCallback(async (agentId: number) => {
    try {
      const response = await fetch(`/api/agent_preferences/${agentId}`);
      if (!response.ok) {
        throw new Error(`Failed to fetch agent preferences. Status: ${response.status}`);
      }
      const data = await response.json();
      ...
      toast({
        variant: 'destructive',
        title: 'API Error',
        description: error.message || 'Could not fetch agent preferences.'
      });
      return null;
    }
  }, [toast]);

  const updateAgentPreferences = useCallback(async (agentId: number, preferences: any) => {
    try {
      const response = await fetch(`/api/agent_preferences/${agentId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        throw new Error(`Failed to update agent preferences. Status: ${response.status}`);
      }
      const data = await response.json();
      toast({
        title: 'Settings Saved',
        description: 'Agent preferences updated successfully.'
      });
      return data.preferences;
    } catch (error: any) {
      ...
      toast({
        variant: 'destructive',
        title: 'API Error',
        description: error.message || 'Could not update agent preferences.'
      });
      return null;
    }
  }, [toast]);

  const archiveAgent = useCallback(async (agentId: number) => {
    try {
      const response = await fetch(`/api/agents/${agentId}/archive`, {
        method: 'PUT',
      });
      if (!response.ok) {
        throw new Error(`Failed to archive agent. Status: ${response.status}`);
      }
      const data = await response.json();
      toast({
        title: 'Agent Archived',
        description: 'Agent has been archived successfully.'
      });
      await forceFetchAgents(); // Refresh the agent list
      return data.agent;
    } catch (error: any) {
      ...
      toast({
        variant: 'destructive',
        title: 'API Error',
        description: error.message || 'Could not archive agent.'
      });
      return null;
    }
  }, [toast, forceFetchAgents]);

  const unarchiveAgent = useCallback(async (agentId: number) => {
    try {
      const response = await fetch(`/api/agents/${agentId}/unarchive`, {
        method: 'PUT',
      });
      if (!response.ok) {
        throw new Error(`Failed to unarchive agent. Status: ${response.status}`);
      }
      const data = await response.json();
      toast({
        title: 'Agent Restored',
        description: 'Agent has been restored successfully.'
      });
      await forceFetchAgents(); // Refresh the agent list
      return data.agent;
    } catch (error: any) {
      console.error("Unarchive agent error:", error);
      toast({
        variant: 'destructive',
        title: 'API Error',
        description: error.message || 'Could not restore agent.'
      });
      return null;
    }
  }, [toast, forceFetchAgents]);
